    ...
    switch (counter) {
        case 1:
            ...
            if (validation.passed) {
                ...
                .then(res => res.json())
                .then(data => {
                    if (data.msg != 'Coletul este livrat') {
                        ...
                        .then(data => {
                            if (data.msg != 'Nici un rezultat!') {
                                ...
                                //go to next page
                                let content = `<h2>Detalii ordin ridicare</h2><br><p><b>De la:</b> ${doi.value}</p><p><b>Orașul de ridicare:</b> ${cinci.value}</p><p><b>Adresa:</b> ${sase.value}, ${sapte.value}, ${opt.value}</p><p><b>Cod poștal:</b> ${patru.value}</p><p><b>Persoana contact:</b> ${noua.value}</p><p><b>Telefon:</b> ${zece.value}</p><p><b>Adresa e-mail:</b> ${unspe.value}</p><br><br><p>Dacă informațiile nu sunt corecte, apasă butonul "Cancel" și reia completarea formularului!</p>`
        ...
        case 2:
            ...
            if (tosCheck.checked) {
                ...
                .then(data => {
                    ...
                    .then(myBlob => {
                        ...
                        if (window.navigator.msSaveOrOpenBlob) {
                            ...
                            downloadLink.textContent = 'Ordin de ridicare'
                            downloadLink.href = window.URL.createObjectURL(new Blob([myBlob], { type: 'application/pdf' }))
                            downloadLink.download = 'fileName.pdf'
                            let content3 = `<h3>Descarcă ordinul dând click pe link-ul de mai jos:</h3><br><br>`
                ...
                next.textContent = 'Plasează un ordin nou'
                next.addEventListener('click', e => {
                    e.preventDefault()
                    window.location.reload()
                })
            } else {
                let warning = new Showwarning('Este necesar să fii de acord cu termenii și condițiile.', 'danger')
                warning.render()
                warning.removeWarning()
                return false
            }
